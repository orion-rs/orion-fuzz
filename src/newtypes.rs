...
pub mod typedefs {
    ...
    pub fn fuzz_blake2b_secret_key(fuzzer_input: &[u8]) {
        ...
        use orion::hazardous::constants::{BLAKE2B_KEYSIZE, BLAKE2B_BLOCKSIZE};
        ...
        if fuzzer_input.len() < 1 || fuzzer_input.len() > BLAKE2B_KEYSIZE {
            ...
            assert_eq!(&sk.unprotected_as_bytes()[..sk.get_original_length()], fuzzer_input);
        ...
        if sk_rand.is_ok() {
            let sk_actual = sk_rand.unwrap();
            assert_ne!(&sk_actual.unprotected_as_bytes(), &[0u8; BLAKE2B_BLOCKSIZE].as_ref());
    ...
    pub fn fuzz_pbkdf2_password(fuzzer_input: &[u8]) {
        ...
        if fuzzer_input.len() <= SHA512_BLOCKSIZE {
            assert_eq!(&password.unprotected_as_bytes()[..fuzzer_input.len()], fuzzer_input);
        } else {
            let digest = orion::hazardous::hash::sha512::digest(fuzzer_input).unwrap();
            assert_eq!(&password.unprotected_as_bytes()[..SHA512_OUTSIZE], digest.as_bytes());
        }
        ...
        if password_rand.is_ok() {
            ...
            assert_ne!(&password_actual.unprotected_as_bytes(), &[0u8; SHA512_BLOCKSIZE].as_ref());
            assert_eq!(password_actual.unprotected_as_bytes().len(), SHA512_BLOCKSIZE);
    ...
    pub fn fuzz_hmac_secret_key(fuzzer_input: &[u8]) {
        ...
        if fuzzer_input.len() <= SHA512_BLOCKSIZE {
            assert_eq!(&sk.unprotected_as_bytes()[..fuzzer_input.len()], fuzzer_input);
        } else {
            let digest = orion::hazardous::hash::sha512::digest(fuzzer_input).unwrap();
            assert_eq!(&sk.unprotected_as_bytes()[..SHA512_OUTSIZE], digest.as_bytes());
        }
        ...
        if sk_rand.is_ok() {
            let sk_actual = sk_rand.unwrap();
            assert_ne!(&sk_actual.unprotected_as_bytes(), &[0u8; SHA512_BLOCKSIZE].as_ref());
